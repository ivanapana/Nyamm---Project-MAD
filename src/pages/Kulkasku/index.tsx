...
const Kulkasku = () => {
  ...
  const handleSave = (updatedIngredient) => {
    Alert.alert('Berhasil', `Bahan "${updatedIngredient.name}" telah disimpan.`);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          >
            <Image
              source={{ uri: 'https://via.placeholder.com/24/FFD700/FFFFFF?text=📦' }}
              style={{ width: 24, height: 24 }}
            />
          </View>
          <View>
            <Text style={{ fontSize: 28, fontWeight: 'bold' }}>{ingredients.length}</Text>
            <Text style={{ fontSize: 16, color: '#666' }}>Total item di Kulkas</Text>
          </View>
        </View>
      </View>
          ...
          {ingredients.map((item) => (
            ...
            >
              ...
              <TouchableOpacity onPress={() => handleEdit(item)}>
                <Image
                  source={{ uri: 'https://via.placeholder.com/24/FFA500/FFFFFF?text=✏️' }}
                  style={{ width: 24, height: 24 }}
                />
              </TouchableOpacity>
            </View>
